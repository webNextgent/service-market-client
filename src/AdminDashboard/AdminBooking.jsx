...
const AdminBooking = () => {
    ...
    const getDemoCoordinates = useCallback((booking) => {
        if (!booking) return { latitude: 23.8103, longitude: 90.4125 }; // Default Dhaka
    ...
    // Get Interactive Map URL - OpenStreetMap with marker
    const getInteractiveMapUrl = useCallback((booking) => {
        const coords = booking?.latitude && booking?.longitude 
            ? { latitude: booking.latitude, longitude: booking.longitude }
            : getDemoCoordinates(booking);
        
        return `https://www.openstreetmap.org/?mlat=${coords.latitude}&mlon=${coords.longitude}#map=15/${coords.latitude}/${coords.longitude}`;
    }, [getDemoCoordinates]);
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-4 text-center">
                                            <p className="text-xs text-gray-500">
                                                Map powered by OpenStreetMap • No API key required
                                            </p>
                                        </div>
                                    </div>
                                </div>
